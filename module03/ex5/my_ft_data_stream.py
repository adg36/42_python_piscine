...
def ft_data_stream() -> None:
    ...
    for event in event_stream(events):
        ...
        print(f"Player {event['player']} (level {event['data']['level']}) killed monster")

    gen = event_stream(events)
    for _ in range(1000):
        print(next(gen))
    ...
    parts = []
    for _ in range(10):
        parts.append(str(next(gen)))
    print(f"Fibonacci sequence (first 10): {', '.join(parts)}")
    ...
    for _ in range(5):
        parts.append(str(next(gen)))
    print(f"Prime numbers (first 5): {', '.join(parts)}")
